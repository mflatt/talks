...
(define (make-plt-title-background plt-red-color plt-blue-color plt-background-color plt-lambda-color
                                   ...
                                   #:full-size? [full-size? #f]
                                   #:clip? [clip? (not full-size?)]
  ...
  (let ()
    ...
    (define raw-background
      (let ([w (if full-size? 1024 client-w)]
            [h (if full-size? 768 client-h)])
    ...
    (define background
      (if full-size?
          (inset raw-background (/ (- client-w 1024) 2) (/ (- client-h 768) 2))
          raw-background))
