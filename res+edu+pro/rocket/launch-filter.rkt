...
(define bm (read-bitmap "launch-raw.png"))
...
(let ([c (make-object color%)])
  (for* ([i (in-range 0 w)]
    ...
    (unless (and (<= lx i rx)
      ...
      (define a (* (- 1.0 (/ (+ (send c red) (send c green)(send c blue))
                             (* 3 255.0)))
                   ;; Decay shadow more quickly
                   (max 0
                        (- 1.0
                           (cond
                             [(<= lx i rx)
                              (/ (min (abs (- j ty)) (abs (- j by)))
                                 ty)]
                             [(<= ty j by)
                              (/ (min (abs (- i lx)) (abs (- i rx)))
                                 lx)]
                             [else
                              (max (/ (min (abs (- j ty)) (abs (- j by)))
                                      ty)
                                   (/ (min (abs (- i lx)) (abs (- i rx)))
                                      lx))])))))
...
(send bm save-file "launch.png" 'png)
