...
(define (draw-single dc dx dy main-color left-color right-color rot)
  (let* ([path (rotate-path body-path rot)]
    ...
    (let-values ([(sx sy) (send dc get-scale)])
      ...
      (send dc set-scale (* sy scale) (* sx scale))
      
      (let-values ([(x y w h) (send path get-bounding-box)])
        (let ([dx (+ (/ dx scale) (- (/ (/ bitmap-size scale) 2) (/ w 2)))]
              [dy (+ (/ dy scale) (- (/ (/ bitmap-size scale) 2) (/ h 2)))])
...
(define draw-honu
  (lambda (dc dx dy)
    (draw-single dc dx dy "black" "red" "blue" honu-rotation)))
